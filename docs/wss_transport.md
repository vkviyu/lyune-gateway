...
> 与混合 soak。人工浏览器已确认共享历史和 WSS→Raw 实时推送，完整 UI 清单仍开放；
> 当前先暂停演进并进行源码审计。
...
用户持久化的历史、WSS 消息能实时到达 Raw 会话。因为 Raw 的首条消息发生在 WSS 登录
之前，这一观察不能替代 Raw→WSS 同时在线实时推送证据；错误密码、注册、建群/入群也
尚未形成完整人工记录。当前先按项目决定进行源码审计，不进入远程 Linux。
