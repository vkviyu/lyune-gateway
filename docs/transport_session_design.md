...
> 的 Mac 自动化 M0–M18 均已通过。人工浏览器已确认共享历史以及 Raw→WSS、
> WSS→Raw 两个方向的同时在线实时消息；完整 UI 操作/负例清单仍单独跟踪。
> 当前先暂停演进并进行源码审计。
...
8. [x] 用户已信任证书，并验证共享历史及 Raw→WSS、WSS→Raw 双向同时在线实时消息；
9. [ ] 错误密码、注册、建群/邀请码入群等完整 UI 操作清单尚未形成逐项人工记录；
10. [ ] 先完成源码逐行审计，再由项目所有者决定是否补齐人工清单、冻结新的 Mac 多传输基线或进入远程 Linux。
