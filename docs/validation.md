...
> 建立日期：2026-08-27
>
> 当前状态：Raw QUIC 与 WSS 的 Mac 自动化 M0–M18 均已通过。用户已在真实浏览器确认共享历史，以及 Raw→WSS、WSS→Raw 两个方向的同时在线实时消息；多传输双向实时互通的人工证据已经关闭，完整 UI 操作/负例清单仍单独跟踪。当前先暂停验证扩展，进入源码逐行审计；不进入远程 Linux，也不创建 release。
...
截至 2026-08-29，commit `425f2a6` 的原始 Raw QUIC M0–M18 和之后的 TransportSession Raw-only M0–M18 均已满足。2026-08-30 加入 WSS 后，又独立完成真实 QUIC↔WSS 混合用户闭环、协议并发/取消、畸形输入、慢消费者、断线补偿、presence、120 秒长流、双 Worker、反复进程故障、混合 soak 与干净 M18 自动化总复跑。2026-08-31 至 09-01 的人工浏览器操作又证明 WSS 与 Raw 共享用户/历史路径，并且双方同时在线时两个方向的新消息都能实时到达。多传输双向实时互通的人工证据已经关闭；错误密码、注册和建群/入群仍未形成完整的逐项人工记录。
...
| Raw→WSS 同时在线实时推送 | 已完成 | 双方同时在线时，Raw 用户发送的新消息由 WSS 浏览器页面实时收到，无需刷新或重读历史 |
| 错误密码、注册、建群/邀请码入群 | 待完整人工记录 | 自动化已覆盖，但仍需 UI 级证据才能关闭人工清单 |

双向实时互通已经关闭；剩余 UI 操作清单不再自动触发下一阶段。当前先执行 [源码逐行审计](code_audit.md)，之后再决定补齐人工清单、修改实现或进入 Linux 的顺序。
...
- 后续人工确认（2026-09-01）：Raw 与 WSS 双方同时在线时，Raw 用户又发送了新消息，WSS 浏览器页面无需刷新或重新读取历史即可实时看到。至此 Raw→WSS 与 WSS→Raw 两个方向都由真实页面操作确认。
- 能证明：真实证书与浏览器 WSS 握手成功；两种 transport binding 汇入同一用户认证、群路由、SQLite 历史和消息投递路径；Raw↔WSS 双向同时在线实时投递成功。
- 尚未逐项记录：错误密码、注册、建群和邀请码入群的完整 UI 操作清单；这些不影响“双向实时互通已通过”的结论。
- 结果：TRANSPORT PASS / UI CHECKLIST PARTIAL。多传输双向实时链路已由真实用户确认；当前按项目决定暂停继续扩展测试，先进入源码审计。
