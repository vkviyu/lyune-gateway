...
> soak 仍待执行，不能由 macOS 单元测试替代；当前先暂停 Linux 阶段并进行源码审计。
>
...
CID v1 的当前候选格式固定为 12 字节。项目尚未发布，因此这里不是对外兼容承诺。QUIC 允许 0–20 字节 CID；短头包不携带 CID 长度，
因此整个集群必须统一使用该长度和版本：
...
当前 12 字节布局是候选首发格式，线路标识为 `version = 1`，不存在需要兼容的旧 CID。
解码器和 reuseport 分类器都严格拒绝其他版本；正式发布前仍可破坏性调整，届时同步更新版本、实现与文档。发布后若出现不兼容布局，再递增版本并单独设计升级策略。
...
8. [部分完成] 文档与真实 UDP loopback 测试已完成；Linux cBPF 实机测试、两个完整网关进程的
   QUIC 跨节点转发和长时间 netem soak 仍属于未来生产采用前的验证。
