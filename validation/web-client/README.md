...
浏览器本身不是 `lyune/2` QUIC 端点。页面通过 `/api` 控制相邻的 `validation/client-agent`；每个页面会话由 agent 建立独立 QUIC 连接，经 Gateway 到 Reactor 完成 auth 和 `.service(1,2)` 命令，并监听 Reactor 主动发出的 `.peer` 流。完整拓扑、启动顺序和判定标准见 `../../docs/validation.md`。
