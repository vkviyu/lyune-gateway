...
//! ## 两条主线
//!
//! 1. **一条传输流 = 一次交换**：Raw QUIC stream 或 WSS logical stream 的首帧是 OPEN，声明目的地；后续帧是 DATA，
//!    只带长度；带 `eof` 的帧是最后一帧。目的地是流级属性，不在每帧重复，
//!    因此伪造后续帧也改不了投递目标。
//! 2. **不重复 binding 已有的能力**：多路复用交给 stream id，请求/响应关联交给
//!    双向流，流内有序与传输反馈交给 Raw QUIC 或 WSS binding。所以帧头里没有通道号、
//!    没有序号、没有窗口，也没有魔数——连接级 ALPN/subprotocol 已保证协议一致。
//!
//! ## 帧结构
//!
//! ```
//! OPEN（一次交换的第一帧，8 字节）
//! ┌────────────┬───────┬────────────────┬───────────┬───────────────┬───────┬───────────┐
//! │ frame_type │ flags │ body_len (u16) │ dest_kind │ response_mode │ group │ route_key │
